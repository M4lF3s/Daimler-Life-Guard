                       ...
                       NUI_IMAGE_TYPE depthType, NUI_IMAGE_RESOLUTION depthRes, BOOL bNearMode, BOOL bFallbackToDefault, NUI_IMAGE_TYPE colorType, NUI_IMAGE_RESOLUTION colorRes, BOOL bSeatedSkeletonMode)
{
	...
	if(FSDKE_OK != FSDK_ActivateLibrary("NdNtaV2ZBZlkExmUAKr7dE7UNsfqqsCMchpFG9KCSZ/ceziJk+oiCd/UiP3wQ+7uS+Oe1u2BPyERGa+cHfhdbgjsIhVfxVcnE+fTP39ANX83h3QB+cwLfTZNkqHU6p6pr+UH9XuNGCKsjuUbQfzb4T4P4b8nTTJIMFgySiwI2c8=")) {
	...
	FSDK_CreateTracker(&faceSdkTracker);

	int err = 0; // set realtime face detection parameters
	FSDK_SetTrackerMultipleParameters(faceSdkTracker, "RecognizeFaces=false; DetectExpression=true; HandleArbitraryRotations=true; DetermineFaceRotationAngle=false; InternalResizeWidth=300; FaceDetectionThreshold=3;", &err);
    ...
    if (m_KinectSensorPresent && m_KinectSensor.GetVideoBuffer())
    {
        ...
        if (SUCCEEDED(hrCopy))
        {
			...
			if(faceCount > 0) {
				...
				int res = FSDK_GetTrackerFacialAttribute(faceSdkTracker, 0, IDs[0], "Expression", AttributeValues, sizeof(AttributeValues));
